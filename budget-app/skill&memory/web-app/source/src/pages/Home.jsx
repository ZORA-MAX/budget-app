...
import ImportDataInspector from '../components/ImportDataInspector'
...
import { extractReceiptImages } from '../lib/receipt-extractor'
import { reconcileReceiptTransactions } from '../lib/receipt-reconciliation'
import {
...
const SOURCE_LABELS = { wechat: '微信', alipay: '支付宝', pdd: '拼多多', bank: '银行卡', cashbook: '记账本', combined: '综合账单' }
...
function scopedOverrides(transactions, overrides) {
  return Object.fromEntries(transactions.map(tx => [txKey(tx), overrides[txKey(tx)]]).filter(([, override]) => override))
}
...
export default function Home({ onDataSaved }) {
  ...
  const [receiptTxs, setReceiptTxs] = useState([])
  const [extractingReceipts, setExtractingReceipts] = useState(false)
  const [receiptProgress, setReceiptProgress] = useState({ current: 0, total: 0, filename: '' })
  ...
  const handleFiles = useCallback(async selectedFiles => {
    ...
    const results = await Promise.allSettled(batch.map(async file => {
      const txs = (await parseBillFile(file)).map(tx => ({ ...tx, sourceFile: file.name }))
      if (txs.length === 0) throw new Error('没有识别到账单流水')
  ...
  const handleReceiptFiles = useCallback(async selectedFiles => {
    setError('')
    setExtractingReceipts(true)
    try {
      const extracted = await extractReceiptImages(selectedFiles, setReceiptProgress)
      if (extracted.length === 0) throw new Error('截图中没有识别到完整交易')
      setReceiptTxs(previous => [...previous, ...extracted])
    } catch (err) {
      setError(`截图识别失败：${err.message}`)
    } finally {
      setExtractingReceipts(false)
      setReceiptProgress({ current: 0, total: 0, filename: '' })
    }
  }, [])
  ...
  const importedTxs = useMemo(() => [...new Map(
    files.flatMap(file => file.txs).map(tx => [txKey(tx), tx])
  ).values()], [files])

  const receiptReconciliation = useMemo(
    () => reconcileReceiptTransactions(importedTxs, receiptTxs),
    [importedTxs, receiptTxs],
  )

  const allTxs = useMemo(() => {
    const merged = applyTransactionMergeMemory([...receiptReconciliation.transactions, ...manualTxs])
    return merged.filter(tx => !deletedKeys.has(txKey(tx)))
  }, [receiptReconciliation, manualTxs, deletedKeys])
  ...
  return (
    <div>
        ...
        <div className="flex items-center justify-between rounded-xl bg-brand-faint px-4 py-3 mb-2">
          <div>
            <div className="text-sm font-medium text-brand">已选择 {files.length} 个账单文件</div>
              <div className="text-xs text-ink-secondary mt-0.5">共识别 {files.reduce((sum, file) => sum + file.txs.length, 0)} 笔完整流水</div>
          </div>
          <button type="button" onClick={() => { setFiles([]); setReceiptTxs([]); setAnalyzed(false) }}
      ...
      <ImportDataInspector files={files} />

      <ReceiptUploadPanel extracting={extractingReceipts} progress={receiptProgress} receipts={receiptTxs}
        matchedCount={receiptReconciliation.matches.length} onFiles={handleReceiptFiles} onClear={() => setReceiptTxs([])} />
      ...
      <div className="border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors mb-2
        ...
        <div className="text-xs text-ink-secondary mt-1">一次可多选多个 CSV、XLS 或 XLSX 文件；保留收入、支出、退款及账单原始字段</div>
        <div className="flex justify-center gap-2 mt-3">
          ...
          <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 text-amber-700">🏦 银行卡</span>
      ...
      {files.length > 0 || receiptTxs.length > 0 ? (
        <button onClick={handleAnalyze} disabled={importingCount > 0}
          ...
          {importingCount > 0 ? '正在读取文件…' : `生成完整收支报告（${allTxs.length} 笔流水）→`}
        </button>
      ) : (
