...
function EditModal({ txName, txMerchant, txProductName, txDetails, currentAmount, direction = 'expense', count, mergeMemory, currentCatKey, currentSubKey, currentTags, onSave, onDelete, onClose }) {
  ...
  const [merchant, setMerchant] = useState(txMerchant || '')
...
function AddModal({ onAdd, onClose }) {
  ...
  const parsedAmount = Number.parseFloat(amount)
  const canAdd = name.trim() && Number.isFinite(parsedAmount) && parsedAmount > 0 && date
  const handleAdd = () => {
    if (!canAdd) return
    onAdd({
      name: name.trim(),
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="add-transaction-title"
        className="relative bg-white dark:bg-surface-card-dark rounded-2xl w-full max-w-xl max-h-[calc(100vh-32px)] overflow-y-auto"
        style={{ boxShadow: '0 24px 80px rgba(0,0,0,.18)' }} onClick={e => e.stopPropagation()}>
        <div className="px-5 pt-5 pb-3">
          <div id="add-transaction-title" className="text-lg font-semibold text-ink dark:text-white">添加消费明细</div>
          <p className="text-xs text-ink-tertiary mt-1 mb-5">产品、商家和补充明细都会显示在逐笔记录中，也会随历史数据保存。</p>
          <div className="space-y-4">
            <label className="block">
              <span className="block text-sm font-medium text-ink-secondary mb-1.5">消费名称 <span className="text-red-500">*</span></span>
              <input autoFocus type="text" value={name} onChange={e => setName(e.target.value)} placeholder="例如：周末家庭聚餐"
                className="w-full px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" />
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="block">
                <span className="block text-sm font-medium text-ink-secondary mb-1.5">产品 / 服务名称</span>
                <input type="text" value={productName} onChange={e => setProductName(e.target.value)} placeholder="双人套餐、会员续费…"
                  className="w-full px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" />
              </label>
              <label className="block">
                <span className="block text-sm font-medium text-ink-secondary mb-1.5">商家 / 交易对方</span>
                <input type="text" value={merchant} onChange={e => setMerchant(e.target.value)} placeholder="盒马、星巴克、Apple…"
                  className="w-full px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" />
              </label>
            </div>
            <label className="block">
              <span className="block text-sm font-medium text-ink-secondary mb-1.5">消费明细</span>
              <textarea value={details} onChange={e => setDetails(e.target.value)} rows="3" placeholder="规格、数量、用途、同行人，或任何希望用于分析的补充信息"
                className="w-full resize-y px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm leading-relaxed outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" />
            </label>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="block">
                <span className="block text-sm font-medium text-ink-secondary mb-1.5">消费金额 <span className="text-red-500">*</span></span>
                <input type="number" min="0.01" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" step="0.01"
                  className="w-full px-3.5 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/10" />
              </label>
              <label className="block">
                <span className="block text-sm font-medium text-ink-secondary mb-1.5">消费日期 <span className="text-red-500">*</span></span>
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex gap-2 justify-end">
          ...
          <button onClick={handleAdd} disabled={!canAdd} className="px-5 py-2 rounded-xl text-sm font-medium bg-brand text-white disabled:opacity-40 disabled:cursor-not-allowed">添加并保存</button>
        </div>
      </div>
    </div>,
    document.body
...
export default function TransactionList({ transactions, overrides, memory = {}, onOverride, onDelete, onAdd }) {
  ...
  const classified = useMemo(() =>
    transactions.map((tx, i) => {
      ...
      const effectiveTx = {
        ...
        merchant: ov?.editedMerchant ?? tx.merchant ?? '',
        productName: ov?.editedProductName ?? tx.productName ?? '',
  ...
  const filtered = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      list = list.filter(t => [t.name, t.productName, t.merchant, t.details].some(value => String(value || '').toLowerCase().includes(s)))
    }
    return list
              ...
              <div key={tx.idx} data-testid={`transaction-row-${tx.idx}`}
                ...
                <button onClick={() => batchMode ? (tx.isExpense && toggleSelect(tx.idx)) : setEditingIdx(tx.idx)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {tx.details ? (
                      <div className="mt-1 rounded-md bg-gray-50 dark:bg-gray-800/60 px-2 py-1.5 text-[11px] leading-relaxed text-ink-secondary dark:text-gray-300 whitespace-pre-wrap break-words">
                        <span className="text-ink-tertiary">明细：</span>{tx.details}
                      </div>
                    ) : null}
