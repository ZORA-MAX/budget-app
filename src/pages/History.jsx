...
function BackupPanel({ months, status, restoring, onExport, onRestore }) {
    ...
    <div className="bg-white dark:bg-surface-card-dark rounded-xl p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-ink-tertiary mt-1 leading-relaxed">
          完整备份会保存全部账单字段和人工分类。导入时只覆盖备份中同月份的数据，其他月份保留。
        </p>
      </div>
      ...
      <div className="text-[11px] text-ink-tertiary">
        当前本机有 {months.length} 个月数据。建议每次核对完账单后下载一份备份，并存入网盘。
      </div>
    </div>
  )
}
...
export default function History({ refreshKey }) {
  ...
  const handleDelete = useCallback((key) => {
    if (!selectedMonth) return
    const next = txsRef.current.filter(tx => `${tx.date.toISOString()}_${tx.amount}_${tx.name}` !== key)
    ...
    queueSave(async () => {
      await Promise.all([saveMonth(selectedMonth, next), saveOverrides(selectedMonth, nextOverrides)])
    })
  }, [queueSave, selectedMonth])
  ...
  const handleDeleteMonth = async (key) => {
    ...
    setMonths(current => current.filter(month => month !== key))
  ...
  const exportAllData = async () => {
    try {
      ...
      const transactionCount = backup.data.months.reduce((sum, record) => sum + record.transactions.length, 0)
      setBackupStatus({ type: 'success', text: `完整备份已下载：${backup.data.months.length} 个月、${transactionCount} 笔账单。` })
    } catch (error) {
  ...
  const restoreBackup = async (event) => {
    ...
    try {
      ...
      setBackupStatus({ type: 'success', text: `恢复完成：${result.monthCount} 个月、${result.transactionCount} 笔账单。` })
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <BackupPanel months={months} status={backupStatus} restoring={restoring} onExport={exportAllData} onRestore={restoreBackup} />
        ...
        <>
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
            <p className="text-xs text-ink-tertiary leading-relaxed">
              💾 刷新页面不会丢失本机数据；清除浏览器数据或换设备仍会丢失。请定期导出完整备份，并将 JSON 文件保存到 iCloud、百度网盘或其他可靠位置。
            </p>
          </div>
        </>
      )}
